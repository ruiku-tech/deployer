...
class DeployController extends Controller {
  ...
  async getFiles(ctx) {
    ...
    fs.readdir(req.context.fileDir, (err, files) => {
      body = { err, data: files.map((file) => ({ file })) };
    });
  }
  ...
  async getHosts(ctx) {
    ...
    fs.readFile(req.context.hostsFile, "utf-8", (err, data) => {
      body = { err, data: data ? JSON.parse(data) : {} };
    });
  }
  // 获取服务器列表
  async postHosts(ctx) {
  ...
  async getbat(ctx) {
    ...
    fs.readFile(filePath, "utf-8", (err, data) => {
      if (data) {
        body = { err, data: JSON.parse(data) };
      } else {
        body = { err: err || `编排组合[${name}]未找到` };
      }
    });
  }
  // 新增编排
  ...
  async postRun(ctx) {
    ...
    if (!server) {
      return (body = { err: `找不到服务器:${req.body.server}` });
    }
    executer.run
  ...
  async getDeployings(ctx) {
    const { request: req, response: res, body } = ctx;
    body = { data: executer.getDeployings() };
  }
  ...
  async postLogin(ctx) {
    ...
    fs.readFile(req.context.userFile, "utf8", (err, data) => {
      ...
      if (exist) {
        body = { data: req.body.username };
      } else {
        body = { err: { code: "用户名或密码错误" } };
      }
    });
  }
  // 记录列表
